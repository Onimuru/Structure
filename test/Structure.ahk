...
Assert.SetLabel("001")
(struct := Structure(8)).NumPut(0, "Int", 2, "Int", 1)
struct.Size := 12
...
Assert.SetLabel("008")
try {
	struct := struct.NumGet(8, "Struct", 4)  ;* Attempt to create a slice from memory not part of another structure.
}
catch {
	e := "*"
}
...
Assert.SetLabel("009")
(struct := Structure(8)).NumPut(0, "Int", 1, "Int", 1)
...
Assert.SetLabel("011")
struct := StructureFromArray([1, 2, 3, 4, 5])  ;* Create a new struct from an array.
loop ((size := struct.Size)//4) {
...
Assert.SetLabel("012")
struct := StructureFromArray([1, 2, 3, 4, 5], "Int")  ;* Create a new struct from an array.
loop ((size := struct.Size)//4) {
...
Assert.SetLabel("013")
struct := StructureFromStructure(Header(1, 2), Body(3, 4, 5, 6))  ;* Create a new struct from two other structs.
loop ((size := struct.Size)//4) {
...
#HotIf (WinActive(A_ScriptName) || WinActive("Structure.ahk"))
...
Header(number1, number2) {
	(header := Structure(8)).NumPut(0, "Int", number1, "Int", number2)

	return (header)
}

Body(number1, number2, number3, number4) {
	(body := Structure(16)).NumPut(0, "Int", number1, "Int", number2, "Int", number3, "Int", number4)

	return (body)
}

Primary(header, body) {
    return (StructureFromStructure(header, body))
}